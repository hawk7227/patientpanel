...
export default function PatientVideoEmbed({
  ...
  const initializeDailyPrebuilt = useCallback(async () => {
    ...
    try {
      ...
      console.log("Daily: Container dimensions:", prebuiltContainerRef.current?.offsetWidth, "x", prebuiltContainerRef.current?.offsetHeight);
      
      // Build join config - patient joins WITH participant token for private rooms
      const joinConfig: { url: string; userName: string; token?: string } = {
        url: joinUrl,
        userName: patientName,
      };

      // Use patient token if available (required for private rooms)
      if (appointment?.dailyco_patient_token && typeof appointment.dailyco_patient_token === 'string' && appointment.dailyco_patient_token.trim() !== '') {
        joinConfig.token = appointment.dailyco_patient_token;
        console.log("Daily: Using patient token to join private room");
      } else {
        console.warn("Daily: No patient token available — join may fail for private rooms");
      }
