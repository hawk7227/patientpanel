...
export default function PatientVideoEmbed({
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      const diff =
        new Date(appointment.requested_date_time!).getTime() - Date.now();
      if (diff <= 0) {
        ...
        setSize({
          width: Math.max(
            ...
            Math.min(
              ...
              window.innerWidth - posRef.current.x
            )
          ),
          height: Math.max(
            350,
            Math.min(
              ...
              window.innerHeight - posRef.current.y
            )
          ),
        });
      };
  ...
  const initializeDailyPrebuilt = useCallback(async () => {
    ...
    try {
      ...
      frame.on("network-quality-change", (event) => {
        if (event?.threshold) {
          setNetworkQuality(
            event.threshold as "good" | "low" | "very-low"
          );
        }
      });
      ...
      console.log(
        "Daily: Container dimensions:",
        prebuiltContainerRef.current?.offsetWidth,
        "x",
        prebuiltContainerRef.current?.offsetHeight
      );

      // Build join config — patient uses PARTICIPANT token (not owner token)
      const joinConfig: { url: string; userName: string; token?: string } = {
        url: joinUrl,
        userName: patientName,
      };

      // Use patient token for private rooms
      const patientToken = appointment?.dailyco_patient_token;
      if (patientToken && typeof patientToken === "string" && patientToken.trim() !== "") {
        joinConfig.token = patientToken;
        console.log("Daily: Using patient participant token");
      } else {
        console.warn("Daily: No patient token — may fail for private rooms");
      }
  ...
  }, [joinUrl, appointment?.dailyco_patient_token, patientName]);
  ...
  const renderTriggerButton = () => {
    ...
    return (
      <button
        ...
        style={{
          background: "linear-gradient(135deg, #00cba9, #0891b2)",
          color: "white",
          boxShadow: "0 4px 20px rgba(0, 203, 169, 0.3)",
        }}
      >
        <svg
          className="w-5 h-5"
        ...
        {isInCall ? "Return to Call" : "Join Video Call"}
      </button>
    );
  };
  ...
  const renderMinimizedBar = () => (
    ...
    >
      <button
        ...
        className="flex items-center gap-3 bg-[#0a0e14] border border-[#00cba9]/30 rounded-xl px-4 py-3 shadow-lg"
        style={{
          boxShadow: "0 10px 40px rgba(0, 203, 169, 0.2)",
        }}
      >
        <div
          className={`w-3 h-3 rounded-full flex-shrink-0 ${isInCall ? "bg-green-500 animate-pulse" : "bg-yellow-500"}`}
        />
        <span className="text-white font-semibold text-sm">
          {isInCall ? `In Call ${formatTime(callSeconds)}` : "Video Call"}
        </span>
        <svg
          className="w-4 h-4 text-[#00cba9]"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
  ...
  const renderExpandedPanel = () => (
    <div
      style={{
        ...
        overflow: "hidden",
        border: isMobile ? "none" : "1px solid rgba(0, 203, 169, 0.2)",
        background: "#0a0e14",
        boxShadow:
          "0 25px 80px rgba(0, 203, 169, 0.15), 0 10px 40px rgba(0, 0, 0, 0.5)",
      }}
    >
      {/* Header - Draggable */}
      <div
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          <div className="flex items-center flex-shrink-0">
            ...
            <span className="text-[#00cba9] font-bold tracking-wider">
              HEALTH
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 ml-4 bg-slate-800/50 px-3 py-1 rounded-full flex-shrink-0">
            <div
              className={`w-2 h-2 rounded-full flex-shrink-0 ${isInCall ? "bg-green-500" : "bg-yellow-500 animate-pulse"}`}
            />
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          {!isMobile && (
            <span className="text-slate-500 text-xs mr-2">Drag to move</span>
          )}
          <button
          ...
          <button
            onClick={leaveCall}
            className="p-2 hover:bg-red-500/30 rounded-lg text-white"
            title="Close"
          >
            <svg
              className="w-4 h-4"
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-slate-900">
            <div className="text-center p-6">
              ...
              <p className="text-gray-400 mb-6">
                Your video visit with {doctorName}
              </p>
              <button
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-slate-900">
            <div className="text-center p-6">
              ...
              <p className="text-red-400 mb-4">
                {callError || "Failed to join the meeting"}
              </p>
              <button
                onClick={initializeDailyPrebuilt}
      ...
      {isInCall && (
        <div className="flex items-center justify-center gap-3 p-4 bg-[#0a0e14] border-t border-slate-700/50 flex-shrink-0">
          ...
          <button
            onClick={toggleMute}
            className={`p-3 rounded-full transition-all ${isMuted ? "bg-red-500 hover:bg-red-600" : "bg-slate-700 hover:bg-slate-600"}`}
          ...
          <button
            onClick={toggleVideo}
            className={`p-3 rounded-full transition-all ${isVideoOff ? "bg-red-500 hover:bg-red-600" : "bg-slate-700 hover:bg-slate-600"}`}
  ...
  const renderFloatingContent = () => {
    ...
    const keepExpandedMounted =
      panelState === "expanded" || (panelState === "minimized" && isInCall);
  ...
  // Suppress unused var warnings for features that exist but aren't rendered yet
  void networkQuality;
  void isDragging;
  void timeRemaining;
  ...
  return (
    <>
      ...
      {isMounted &&
        portalContainer &&
        panelState !== "closed" &&
        createPortal(renderFloatingContent(), portalContainer)}
    </>
  );
}
