    ...
    <div className="text-white font-sans overflow-hidden" style={{ background: "linear-gradient(168deg, #091211 0%, #080c10 40%, #0a0e14 100%)", height: "100svh", minHeight: "0" }}>
      ...
      <div className="h-full max-w-[430px] mx-auto flex flex-col" style={{ paddingTop: "env(safe-area-inset-top, 4px)", paddingBottom: "env(safe-area-inset-bottom, 4px)", paddingLeft: "16px", paddingRight: "16px" }}>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden pb-1 space-y-2 min-h-0" style={{ scrollbarWidth: "none" }}>
          ...
          {/* STEP 1: Reason for Visit — shows ONLY by itself */}
          {reason ? (
            <button onClick={() => { setReason(""); setChiefComplaint(""); setSymptomsDone(false); saveAnswers({ reason: "", chiefComplaint: "", symptomsDone: false }); }} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] transition-all" style={{ animation: "fadeInPill 0.5s cubic-bezier(0.22, 1, 0.36, 1) both" }}>
              <div className="w-6 h-6 rounded-full bg-gray-700 flex items-center justify-center flex-shrink-0"><Check size={14} className="text-gray-500" strokeWidth={3} /></div>
          ...
          {/* STEP 2: Describe Symptoms — shows ONLY by itself */}
          {reason && symptomsDone ? (
            <button onClick={() => { setChiefComplaint(""); setSymptomsDone(false); saveAnswers({ chiefComplaint: "", symptomsDone: false }); }} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] transition-all" style={{ animation: "fadeInPill 0.5s cubic-bezier(0.22, 1, 0.36, 1) both" }}>
              <div className="w-6 h-6 rounded-full bg-gray-700 flex items-center justify-center flex-shrink-0"><Check size={14} className="text-gray-500" strokeWidth={3} /></div>
              <div className="flex-1 min-w-0 text-left">
                <span className="text-gray-500 text-[11px] font-semibold truncate block">Symptoms</span>
                <span className="text-gray-600 text-[9px] truncate block">{chiefComplaint}</span>
              </div>
              <span className="text-white text-[10px] font-semibold flex-shrink-0">Tap to<br/>change</span>
            </button>
          ) : reason && activeGuideStep === 2 && (
            <div className={`rounded-xl bg-[#11161c] p-4 space-y-3 transition-all ${activeOrangeBorder}`} style={{ animation: "fadeInStep 0.7s cubic-bezier(0.22, 1, 0.36, 1) both" }}>
              <div className="flex items-center justify-center gap-2"><span className="text-[11px] font-black text-[#f97316] bg-[#f97316]/15 w-6 h-6 rounded-full flex items-center justify-center">2</span><span className="text-white text-[15px] font-black uppercase tracking-wide">Describe Your Symptoms</span></div>
              ...
              {chiefComplaint.length >= 10 && (
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => { setSymptomsDone(true); saveAnswers({ chiefComplaint, symptomsDone: true }); }} className="px-5 py-2 rounded-lg bg-[#2dd4a0] text-black text-[13px] font-bold transition-all hover:bg-[#2dd4a0]/90 active:scale-95" style={{ animation: "fadeInBtn 0.4s cubic-bezier(0.22, 1, 0.36, 1) both" }}>Continue →</button>
                </div>
              )}
            </div>
          )}

          {/* STEP 3: Preferred Pharmacy — shows ONLY by itself */}
          {reason && symptomsDone && (pharmacy ? (
            ...
            <div className={`rounded-xl bg-[#11161c] p-4 space-y-2 transition-all ${activeOrangeBorder}`} style={{ animation: "fadeInStep 0.7s cubic-bezier(0.22, 1, 0.36, 1) both" }}>
              <div className="flex items-center gap-3"><span className="text-[11px] font-black text-[#f97316] bg-[#f97316]/15 w-6 h-6 rounded-full flex items-center justify-center">2</span><span className="text-white text-[10px] font-semibold uppercase tracking-wider">Preferred Pharmacy</span></div>
          ...
          {/* STEP 4: Select Visit Type — shows ONLY by itself */}
          ...
          {reason && symptomsDone && pharmacy && (
            visitTypeConfirmed ? (
              ...
              <div className={`rounded-xl bg-[#11161c] p-4 space-y-3 transition-all ${activeOrangeBorder}`} style={{ animation: "fadeInStep 0.7s cubic-bezier(0.22, 1, 0.36, 1) both" }}>
                <div className="flex items-center gap-3"><span className="text-[11px] font-black text-[#f97316] bg-[#f97316]/15 w-6 h-6 rounded-full flex items-center justify-center">3</span><span className="text-white text-[10px] font-semibold uppercase tracking-wider">Select Visit Type</span></div>
