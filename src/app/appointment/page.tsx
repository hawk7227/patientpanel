...
export default function AppointmentProcess() {
  ...
  // Load saved patient data from localStorage (persists across sessions)
  // Then check sessionStorage for current-session email data
  useEffect(() => {
    // 1. Prefill from localStorage (saved from previous successful bookings)
    try {
      ...
      if (savedPatient) {
        ...
        // Auto-trigger email check for saved email
        if (patient.email) {
    ...
    // 2. Override with sessionStorage if present (current session from landing page)
  ...
  const handlePaymentSuccess = async () => {
    ...
    try {
      ...
      // Save patient info to localStorage for future booking autofill
      try {
                  ...
                  <div className="flex justify-center">
                    <button 
                      onClick={async () => {
                        ...
                        // IMPORTANT: Read skipIntake from sessionStorage (set correctly above in email check)
                        // Do NOT use emailExists here — React setState is async, so it's still stale
