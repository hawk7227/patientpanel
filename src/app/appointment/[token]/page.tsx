...
function AppointmentContent() {
  ...
  // =============================================
  // FIX: Build "Open in new tab" URL with patient token
  // The raw Daily.co URL won't work for private rooms.
  // Daily Prebuilt supports ?t=TOKEN query param to authenticate.
  // =============================================
  const buildFallbackVideoUrl = (): string | null => {
    ...
    if (patientToken && patientToken.trim() !== "") {
      // Daily.co Prebuilt supports ?t=<token> for authentication
      const separator = appointment.dailyco_meeting_url.includes("?") ? "&" : "?";
      return `${appointment.dailyco_meeting_url}${separator}t=${patientToken}`;
    }
    // No token available — still return URL but it may fail for private rooms
    ...
    >
      ...
      >
        ...
        <div className="flex-shrink-0 pb-2 pt-1 space-y-1.5">
          ...
          {/* FIX: "Open in new tab" now includes patient token via ?t= param */}
          {fallbackVideoUrl && (
