...
<div className="flex items-center justify-center gap-2 mt-3 mb-2">
              ...
              <div className={`rounded-xl bg-transparent p-3 space-y-2 transition-all ${activeOrangeBorder} flex flex-col min-h-0`}>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => { setSymptomsDone(true); saveAnswers({ chiefComplaint, symptomsDone: true }); }} disabled={chiefComplaint.length < 10} className={`flex-1 py-3 rounded-xl text-white font-bold text-[14px] transition-all active:scale-95 flex items-center justify-center gap-1 border-2 ${chiefComplaint.length >= 10 ? "border-[#f97316] shadow-[0_0_14px_rgba(249,115,22,0.4)]" : "border-[#f97316]/30 opacity-40 cursor-not-allowed"}`} style={{ background: chiefComplaint.length >= 10 ? "rgba(249,115,22,0.18)" : "rgba(249,115,22,0.05)" }}>Continue →</button>
                </div>
              </div>
            </div>
          )}
