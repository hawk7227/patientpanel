...
function InlineCTABlock({ onActivate }: { onActivate: () => void }) {
  const [returningEmail, setReturningEmail] = useState("");
  const [searching, setSearching] = useState(false);
  const [searchError, setSearchError] = useState<string | null>(null);
  ...
  return (
    <div className="flex flex-col items-center gap-5 w-full">
      <button onClick={() => { try { localStorage.removeItem("medazon_express_answers"); sessionStorage.setItem("browserInfo", JSON.stringify(getBrowserInfo())); sessionStorage.setItem("expressPatient", JSON.stringify({ id: null, firstName: "", lastName: "", email: "", phone: "", dateOfBirth: "", address: "", source: "new", pharmacy: "" })); } catch {} onActivate(); }} className="bg-orange-500 text-white font-bold px-8 py-3.5 rounded-xl text-base md:text-xl md:px-10 md:py-5 hover:bg-orange-400 transition-all flex items-center gap-2 w-full sm:w-auto justify-center whitespace-nowrap">
        Book My 1st Visit — $1.89 Reserve Fee <ArrowRight size={20} />
      </button>
      <div className="w-full max-w-lg">
        <div className="text-center mb-3">
  ...
  const [inlineChiefComplaint, setInlineChiefComplaint] = useState("");
        ...
        <div className="max-w-lg mx-auto relative z-10 text-center">
          ...
          {!showInlineStep ? (
            <>
              ...
              <div className="mb-4">
                <InlineCTABlock onActivate={() => setShowInlineStep(true)} />
              </div>
            </>
          ) : (
            /* Inline symptom step — same look as express checkout step 2 */
            <div className="w-full mb-4" style={{ animation: "fadeInStep 0.4s cubic-bezier(0.22,1,0.36,1) both" }}>
              <style>{`@keyframes fadeInStep { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:translateY(0); } }`}</style>
              <div className="rounded-xl p-3 space-y-2 border-2 border-[#f97316]" style={{ background: "transparent" }}>
                <textarea
                  id="inline-symptoms-textarea"
                  value={inlineChiefComplaint}
                  onChange={(e) => setInlineChiefComplaint(e.target.value)}
                  autoFocus
                  placeholder="e.g., Burning during urination for 3 days..."
                  rows={3}
                  className={`w-full bg-[#0d1218] border-2 rounded-xl px-4 py-3 text-sm text-white focus:outline-none resize-none placeholder:text-white/50 caret-white transition-all ${inlineChiefComplaint.length >= 10 ? "border-[#2dd4a0]/30" : "border-[#f97316] focus:border-[#f97316]"}`}
                />
                {inlineChiefComplaint.length < 10 ? (
                  <p className="text-gray-300 text-xs">Type at least <span className="text-[#f97316] font-black text-base inline-block">{10 - inlineChiefComplaint.length}</span> more characters</p>
                ) : (
                  <p className="text-[#2dd4a0] text-xs font-semibold">✓ Ready to continue</p>
                )}
                <div className="flex gap-2">
                  <button onClick={() => { setShowInlineStep(false); setInlineChiefComplaint(""); }} className="flex-1 py-3 rounded-xl text-white font-bold text-sm transition-all flex items-center justify-center gap-1.5 border border-[#2dd4a0]/30" style={{ background: "rgba(45,212,160,0.12)" }}>← Back</button>
                  <button onClick={() => {
                    if (inlineChiefComplaint.length < 10) { document.getElementById("inline-symptoms-textarea")?.focus(); return; }
                    try { localStorage.removeItem("medazon_express_answers"); localStorage.setItem("medazon_express_answers", JSON.stringify({ chiefComplaint: inlineChiefComplaint, symptomsDone: true })); } catch {}
                    window.location.href = "/express-checkout";
                  }} className="flex-1 py-3 rounded-xl text-white font-bold text-sm transition-all flex items-center justify-center gap-1 border-2 border-[#f97316]" style={{ background: "linear-gradient(135deg,#f97316 0%,#ea8a2e 100%)", boxShadow: "0 4px 16px rgba(249,115,22,0.25)" }}>Continue →</button>
                </div>
              </div>
            </div>
          )}
