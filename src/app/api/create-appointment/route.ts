...
export async function POST(request: Request) {
  try {
     ...
     if (isLiveVisit && visitType === "video" && requestedDateTime) {
      ...
      try {
        if (!process.env.DAILY_API_KEY) {
          ...
          // Create participant token for patient (non-owner, can join private room)
          try {
            const patientToken = await dailyService.createMeetingToken({
              properties: {
                room_name: dailyMeeting.name,
                is_owner: false,
                user_name: `${patientFirstName || "Patient"} ${patientLastName || ""}`.trim(),
              },
            });
            console.log("✅ Daily.co patient token created");
            (dailyMeeting as any).patient_token = patientToken.token;
          } catch (ptError) {
            console.error("⚠️ Failed to create patient token:", ptError);
            // Patient won't be able to join private room without token
          }
        }
      } catch (dailyError) {
    ...
    if (dailyMeeting) {
      ...
      appointmentInsert.dailyco_owner_token = (dailyMeeting as any).owner_token; // Store doctor's host token
      if ((dailyMeeting as any).patient_token) {
        appointmentInsert.dailyco_patient_token = (dailyMeeting as any).patient_token; // Store patient's join token
      }
    }
