...
export async function POST(request: Request) {
  try {
    ...
    console.log("[CREATE_APPOINTMENT] Received request with appointment data.", appointmentData ? "Appointment data present." : "No appointment data.");
    ...
    if (patientEmail) {
      ...
      if (patientByEmail && !emailError) {
        ...
        const nameMatches = (!patientFirstName || patientByEmail.first_name === patientFirstName) &&
    ...
    if (!patientData && data.patientId) {
      ...
      if (patientById && !idError) {
        ...
        const emailMatches = !patientEmail || patientById.email?.toLowerCase().trim() === patientEmail.toLowerCase().trim();
        const nameMatches = (!patientFirstName || patientById.first_name === patientFirstName) &&
    ...
    if (patientError || !patientData) {
      ...
      // #region agent log
      fetch('http://127.0.0.1:60000/ingest/9f837d7d-c74d-4d53-b3e3-0fed42051042',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'create-appointment/route.ts:162',message:'Patient not found or mismatch, creating new patient',data:{formEmail:patientEmail,formFirstName:patientFirstName,formLastName:patientLastName,formPhone:patientPhone,existingPatientId:data.patientId},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'D'})}).catch(()=>{});
      // #endregion
      ...
      if (patientError || !patientData) {
        return NextResponse.json(
          { error: "Patient not found", details: "Patient ID and email lookup both failed. Please ensure patient information is provided." },
          { status: 404 }
        );
      }
    }
    ...
    const patientPhone = data.phone || appointmentData?.phone || patientData?.phone || null;
    ...
    const patientTimezone = data.patientTimezone || patientData.timezone || 'America/New_York';
    
    // #region agent log
    fetch('http://127.0.0.1:60000/ingest/9f837d7d-c74d-4d53-b3e3-0fed42051042',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'create-appointment/route.ts:220',message:'Patient timezone resolution',data:{dataPatientTimezone:data.patientTimezone,patientDataTimezone:patientData.timezone,resolvedPatientTimezone:patientTimezone},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
    // #endregion
    ...
    if (doctorInfo?.timezone && doctorInfo.timezone !== "America/Phoenix") {
      fetch('http://127.0.0.1:60000/ingest/9f837d7d-c74d-4d53-b3e3-0fed42051042',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'create-appointment/route.ts:269',message:'Doctor timezone mismatch',data:{doctorRecordTimezone:doctorInfo.timezone,usingTimezone:providerTimezone},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
    }
    // #endregion
    ...
    if (data.appointmentDate && data.appointmentTime) {
      ...
      const phoenixDay = parseInt(phoenixParts.find(p => p.type === 'day')?.value || '0');
      const phoenixHour = parseInt(phoenixParts.find(p => p.type === 'hour')?.value || '0');
      const phoenixMinute = parseInt(phoenixParts.find(p => p.type === 'minute')?.value || '0');
      ...
      for (let offsetHours = -15; offsetHours <= 15; offsetHours++) {
        ...
        const testMonth = parseInt(testPhoenixParts.find(p => p.type === 'month')?.value || '0');
        const testDay = parseInt(testPhoenixParts.find(p => p.type === 'day')?.value || '0');
        const testHour = parseInt(testPhoenixParts.find(p => p.type === 'hour')?.value || '0');
        const testMinute = parseInt(testPhoenixParts.find(p => p.type === 'minute')?.value || '0');
        
        if (testYear === phoenixYear && testMonth === phoenixMonth && testDay === phoenixDay && 
            testHour === phoenixHour && testMinute === phoenixMinute) {
    ...
    const appointmentInsert: Record<string, unknown> = {
      ...
      // Payment information
      payment_intent_id: payment_intent_id || (isTestMode ? `pi_test_${Date.now()}` : null),
    ...
    const chiefComplaintText = `${data.symptoms} / ${data.chief_complaint}` || null;
    if (chiefComplaintText && appointment?.id && appointment?.patient_id) {
      try {
        ...
        if (notesError) {
          console.error("[CLINICAL_NOTES] ❌ Error saving chief_complaint:", notesError);
        }
      } catch (clinicalNotesError) {
        console.error("[CLINICAL_NOTES] ❌ Error saving chief_complaint to clinical_notes:", clinicalNotesError);
      }
    }
    ...
    if (data.allergies === true && data.allergiesDetails && appointment?.id && appointment?.patient_id) {
      try {
        console.log("Inside the if condition")
        // Convert to string if it's not already
        const allergiesDetailsStr = typeof data.allergiesDetails === 'string' 
        ...
        if (allergyList.length > 0) {
          ...
          if (fetchError) {
            console.error("[ALLERGIES] ❌ Error fetching existing allergies:", fetchError);
          } else {
            console.log('[ALLERGIES] Found existing allergies:', existingAllergies?.length || 0);
          }

          // Filter out duplicates (case-insensitive)
          const existingAllergenNames = (existingAllergies || []).map((a: { allergen_name?: string }) => a.allergen_name?.toLowerCase());
          const newAllergies = allergyList.filter((allergen: string) => 
            !existingAllergenNames.includes(allergen.toLowerCase())
          );

          console.log('[ALLERGIES] New allergies to insert:', newAllergies.length);
          ...
          if (newAllergies.length > 0) {
            ...
            if (insertError) {
              console.error("[ALLERGIES] ❌ Error saving allergies to patient_allergies:", insertError);
            } else {
              console.log(`[ALLERGIES] ✅ Saved ${newAllergies.length} new allergies for patient ${appointment.patient_id}`);
            }
          } else {
            console.log("[ALLERGIES] ℹ️ All allergies already exist for this patient, skipping insert");
          }
        } else {
          console.log('[ALLERGIES] No allergies to process (empty list)');
        }
      } catch (allergiesError) {
        console.error("[ALLERGIES] ❌ Error saving allergies to patient_allergies:", allergiesError);
      }
    } else {
    ...
    if (data.surgeries === true && data.surgeriesDetails && appointment?.id && appointment?.patient_id) {
      try {
        const surgeriesContent = typeof data.surgeriesDetails === 'string' 
          ? data.surgeriesDetails 
          : String(data.surgeriesDetails || '');
        
        console.log('[SURGERIES] Surgeries content:', surgeriesContent.substring(0, 100) + '...');
        const { error: insertError } = await supabase
          .from('clinical_notes')
          .insert({
            patient_id: appointment.patient_id,
        ...
        if (insertError) {
          console.error("[SURGERIES] ❌ Error saving surgeries to clinical_notes:", insertError);
        } else {
          console.log(`[SURGERIES] ✅ Saved surgeries note for appointment ${appointment.id}`);
        }
      } catch (surgeriesError) {
        console.error("[SURGERIES] ❌ Error saving surgeries to clinical_notes:", surgeriesError);
      }
    } else {
    ...
    if (data.medicalIssues === true && data.medicalIssuesDetails && appointment?.id && appointment?.patient_id) {
      console.log('[MEDICAL_ISSUES] Medical issues data found, processing...');
      try {
        // Convert to string if it's not already
        const medicalIssuesDetailsStr = typeof data.medicalIssuesDetails === 'string' 
        ...
        if (issuesList.length > 0) {
          ...
          if (fetchError) {
            console.error("[MEDICAL_ISSUES] ❌ Error fetching existing problems:", fetchError);
          } else {
            console.log('[MEDICAL_ISSUES] Found existing problems:', existingProblems?.length || 0);
          }

          // Filter out duplicates (case-insensitive)
          const existingProblemNames = (existingProblems || []).map((p: { problem_name?: string }) => p.problem_name?.toLowerCase());
          const newProblems = issuesList.filter((issue: string) => 
            !existingProblemNames.includes(issue.toLowerCase())
          );

          console.log('[MEDICAL_ISSUES] New problems to insert:', newProblems.length);
          ...
          if (newProblems.length > 0) {
            ...
            console.log('[MEDICAL_ISSUES] Inserting problems:', problemsToInsert);
            const { error: insertError } = await supabase
              .from('problems')
              .insert(problemsToInsert);

            if (insertError) {
              console.error("[MEDICAL_ISSUES] ❌ Error saving medical issues to problems:", insertError);
            } else {
              console.log(`[MEDICAL_ISSUES] ✅ Saved ${newProblems.length} new problems for patient ${appointment.patient_id}`);
            }
          } else {
            console.log("[MEDICAL_ISSUES] ℹ️ All medical issues already exist for this patient, skipping insert");
          }
        } else {
          console.log('[MEDICAL_ISSUES] No issues to process (empty list)');
        }
      } catch (medicalIssuesError) {
        console.error("[MEDICAL_ISSUES] ❌ Error saving medical issues to problems:", medicalIssuesError);
      }
    } else {
      console.log('[MEDICAL_ISSUES] Skipping medical issues - Reason:', {
        medicalIssues: data.medicalIssues,
        medicalIssuesDetails: data.medicalIssuesDetails ? 'provided' : 'missing',
        appointmentId: appointment?.id ? 'exists' : 'missing',
        patientId: appointment?.patient_id ? 'exists' : 'missing'
      });
    }
    ...
    if (data.medications === true && data.medicationsDetails && appointment?.id && appointment?.patient_id) {
      console.log('[MEDICATIONS] Medications data found, processing...');
      try {
        // Convert to string if it's not already
        const medicationsDetailsStr = typeof data.medicationsDetails === 'string' 
        ...
        if (medicationList.length > 0) {
          ...
          const { data: existingMedications, error: fetchError } = await supabase
          ...
          if (fetchError) {
            console.error("[MEDICATIONS] ❌ Error fetching existing medications:", fetchError);
          } else {
            console.log('[MEDICATIONS] Found existing medications:', existingMedications?.length || 0);
          }

          // Filter out duplicates (case-insensitive)
          const existingMedicationNames = (existingMedications || []).map((m: { medication_name?: string }) => m.medication_name?.toLowerCase());
          const newMedications = medicationList.filter((medication: string) => 
            !existingMedicationNames.includes(medication.toLowerCase())
          );

          console.log('[MEDICATIONS] New medications to insert:', newMedications.length);

          if (newMedications.length > 0) {
            const medicationsToInsert = newMedications.map((medication: string) => ({
              patient_id: appointment.patient_id,
            ...
            console.log('[MEDICATIONS] Inserting medications:', medicationsToInsert);
            const { error: insertError } = await supabase
              .from('medication_history')
              .insert(medicationsToInsert);

            if (insertError) {
              console.error("[MEDICATIONS] ❌ Error saving medications to medication_history:", insertError);
            } else {
              console.log(`[MEDICATIONS] ✅ Saved ${newMedications.length} new medications for patient ${appointment.patient_id}`);
            }
          } else {
            console.log("[MEDICATIONS] ℹ️ All medications already exist for this patient, skipping insert");
          }
        } else {
          console.log('[MEDICATIONS] No medications to process (empty list)');
        }
      } catch (medicationsError) {
        console.error("[MEDICATIONS] ❌ Error saving medications to medication_history:", medicationsError);
      }
    } else {
    ...
    const paymentCurrency = paymentIntent?.currency || (isTestMode ? 'USD' : null);
    ...
    if (requestedDateTime) {
      ...
      const patientTZ = patientTimezone || data.patientTimezone || "America/New_York";
    ...
    if (doctorPhone) {
      try {
        await sendSMS({
          to: doctorPhone,
          message: `New appointment scheduled: ${patientName} on ${formattedDate} at ${formattedTime}. ${visitTypeDisplay}. View: ${doctorPanelLink}${zoomStartUrl ? `\n\n⚠️ WARNING: START meeting link (host only, do NOT share with patients) and this will start meeting instantly: ${zoomStartUrl}` : ''}`,
        });
    ...
    // ============================================================
    const cdssPromise = generateCDSSInBackground(supabase, appointment, data, chiefComplaintText)
      .catch((err) => console.error('[CDSS_PRE_GENERATE] Background task error:', err));
    
    if (waitUntilFn) {
      // On Vercel: waitUntil keeps the serverless function alive
      // Even if patient closes browser, server continues generating CDSS
      console.log('[CDSS_PRE_GENERATE] Using waitUntil for reliable background processing');
      waitUntilFn(cdssPromise);
    } else {
      // Local dev or other hosts: promise runs but may not complete
      console.log('[CDSS_PRE_GENERATE] waitUntil not available, using fire-and-forget');
    }
...
interface CDSSResponse {
  ...
  templates: { hpi: string; ros_general: string; assessment: string; plan: string };
  medication_suggestions: {
    medications: Array<{ medication: string; sig: string; quantity: string; refills: number; notes: string }>;
    safety_notes: string[];
  };
  soap_note: { chief_complaint: string; hpi: string; ros: string; assessment: string; plan: string };
...
function buildClinicalContext(input: CDSSInput): string {
  ...
  if (input.medicationHistory && input.medicationHistory.length > 0) {
    sections.push(`\n## CURRENT MEDICATIONS`);
    input.medicationHistory.forEach(med => sections.push(`- ${med.medication}`));
  }
...
function extractRedFlags(text: string): string[] {
  ...
  const redFlagKeywords = [
    ...
    'high fever', 'severe bleeding', 'severe allergic reaction'
  ];
...
function validateAndEnrichResponse(response: any, input: CDSSInput): CDSSResponse {
  ...
  return {
    ...
    soap_note: response.soap_note || { chief_complaint: '', hpi: '', ros: '', assessment: '', plan: '' },
    ...
    if (promptId) {
      try {
        ...
        const response = await Promise.race([
          ...
          new Promise((_, reject) => setTimeout(() => reject(new Error('Prompt API timeout')), 90000))
        ]) as any;
        ...
        console.log('[CDSS] ⚠️ Prompt API failed, using Chat Completions:', promptError.message);
      }
    }
...
Respond in JSON format:
{
  ...
  "follow_up_recommendations": "..."
    ...
    return {
      classification: { category: 'General Consultation', description: 'Requires clinical review' },
      risk_level: 'moderate_risk',
      risk_factors: [],
      allergy_alerts: input.patientIntake?.hasDrugAllergies ? [`⚠️ Patient reports drug allergies: ${input.patientIntake.allergies}`] : [],
      interaction_alerts: [],
      templates: { hpi: '', ros_general: '', assessment: '', plan: '' },
      medication_suggestions: { medications: [], safety_notes: ['AI generation failed - manual review required'] },
      soap_note: { chief_complaint: input.chief_complaint || '', hpi: '', ros: '', assessment: '', plan: '' },
      clinical_pearls: [],
      follow_up_recommendations: '',
      analysis_summary: 'CDSS generation failed - manual clinical assessment required'
    };
  }
}
...
async function generateCDSSInBackground(
  ...
  try {
    console.log('[CDSS_PRE_GENERATE] 🔄 Starting CDSS generation for appointment:', appointment.id);

    const openaiApiKey = process.env.OPENAI_API_KEY;
    if (!openaiApiKey) {
      console.log('[CDSS_PRE_GENERATE] ⚠️ OpenAI API key not configured, skipping');
      return;
    }
    ...
    const cdssInput: CDSSInput = {
      ...
      duration: '',
      severity: '',
      allergies: intakeData.allergies === true && intakeData.allergiesDetails ? intakeData.allergiesDetails : 'NKDA',
      ...
      medicationHistory: intakeData.medications === true && intakeData.medicationsDetails
        ? intakeData.medicationsDetails.split(/[,\n]/).map((m: string) => ({ medication: m.trim() })).filter((m: { medication: string }) => m.medication)
        : [],
      patientInfo: {
    ...
    if (saveError) {
      ...
      if (flagError) {
        console.error('[CDSS_PRE_GENERATE] ⚠️ Failed to set auto-generated flag:', flagError);
      } else {
