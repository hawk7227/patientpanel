...
export async function POST(request: Request) {
  try {
      ...
      try {
        if (!process.env.DAILY_API_KEY) {
          ...
          try {
            ...
            dailyMeeting.patient_token = patientToken.token;
          } catch (ptError) {
    ...
    if (dailyMeeting) {
      ...
      if (dailyMeeting.patient_token) {
        appointmentInsert.dailyco_patient_token = dailyMeeting.patient_token; // Store patient's join token
      }
    }
