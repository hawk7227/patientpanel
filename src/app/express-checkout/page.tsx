...
<div className="flex items-center justify-center gap-2 mt-3 mb-2">
              ...
              <div className={`rounded-xl bg-transparent p-3 space-y-2 transition-all ${activeOrangeBorder} flex flex-col min-h-0`}>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => { setSymptomsDone(true); saveAnswers({ chiefComplaint, symptomsDone: true }); }} disabled={chiefComplaint.length < 10} className={`flex-1 py-3 rounded-xl text-white font-bold text-[14px] transition-all active:scale-95 flex items-center justify-center gap-1 border-2 ${chiefComplaint.length >= 10 ? "border-[#2dd4a0]/30" : "border-[#f97316] opacity-50 cursor-not-allowed"}`} style={{ background: "#f97316", animation: chiefComplaint.length >= 10 ? "guidePulse 2s ease-in-out infinite" : "none" }}>Continue →</button>
                </div>
              </div>
            </div>
          )}
