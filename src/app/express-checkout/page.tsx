...
export default function ExpressCheckoutPage() {
  ...
  const goBack = useCallback(() => {
    ...
    if (step === 4) {
      ...
      // Abort any in-flight payment intent
      paymentFetchController.current?.abort();
      setClientSecret("");
      setPaymentIntentError(null);
      saveAnswers({ pharmacy: "", pharmacyAddress: "", pharmacyInfo: null, visitTypeConfirmed: false, visitTypePopup: null, appointmentDate: "", appointmentTime: "" });
      return;
    }
    if (step === 5) {
    ...
    <div className="ec-root text-white font-sans overflow-hidden" style={{ background: "radial-gradient(900px 420px at 18% 12%, rgba(255,179,71,0.18), transparent 55%), radial-gradient(800px 380px at 76% 22%, rgba(110,231,183,0.16), transparent 55%), linear-gradient(180deg, #0b0f0c 0%, #070a08 100%)", height: "100dvh", minHeight: "100vh" }}>
      ...
      <div className="h-full max-w-[430px] mx-auto flex flex-col" style={{ paddingTop: "env(safe-area-inset-top, 4px)", paddingBottom: "env(safe-area-inset-bottom, 4px)", paddingLeft: "16px", paddingRight: "16px" }}>
        ...
        <div className={`text-center flex-shrink-0 ${headerUltraCompact ? "pt-1 pb-1" : headerIsStep5 ? "pt-2 pb-1" : "pt-3 pb-2"}`}>
          ...
          <h1 className={`text-white font-black leading-tight ${headerIsStep5 ? "mb-1" : "mb-2"}`} style={{ fontSize: headerIsStep5 ? "clamp(22px, 6.2vw, 28px)" : "clamp(24px, 7vw, 30px)" }}>
            {headerIsStep5 ? <>You&apos;re All <span className="text-[#2dd4a0]">Set.</span></> : <>Book your <span className="text-[#2dd4a0]">visit.</span></>}
          </h1>          {/* Badge — changes on Step 5 */}
          {!headerUltraCompact && (
          <div className={`inline-flex items-center gap-1.5 rounded-full border border-[#2dd4a0]/30 bg-[#2dd4a0]/10 ${headerIsStep5 ? "px-3 py-1" : "px-4 py-1.5"}`}>
            {headerIsStep5 ? (
        ...
        <div className={`flex-shrink-0 ${headerIsStep5 ? "pb-1" : "pb-1.5"}`}>
          ...
          {/* Speaking pill (step-aware) */}
          <div className="mt-2 flex justify-center">
            <div
              key={pillText}
              className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[12px] text-white/80 animate-[pillIn_200ms_ease-out]"
            >
...
<div className="flex items-center justify-center gap-2 mt-3 mb-2">
          ...
          {uiStep === 5 && (
            <div style={{ animation: "fadeInStep 0.7s cubic-bezier(0.22, 1, 0.36, 1) both" }}>
              <div className="flex items-center justify-center gap-2 mt-3 mb-2">
                <span className="w-2.5 h-2.5 rounded-full bg-[#f97316]" />
                <span className="text-white text-[15px] font-black uppercase tracking-wide">Confirm &amp; Pay</span>
              </div>
              <div className={`rounded-xl bg-transparent p-4 space-y-3 transition-all ${activeOrangeBorder}`}>
                {/* Compact summary */}
