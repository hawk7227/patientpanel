    ...
    <div className="text-white font-sans overflow-hidden" style={{ background: "radial-gradient(900px 420px at 18% 12%, rgba(255,179,71,0.18), transparent 55%), radial-gradient(800px 380px at 76% 22%, rgba(110,231,183,0.16), transparent 55%), linear-gradient(180deg, #0b0f0c 0%, #070a08 100%)", height: "100svh", minHeight: "0" }}>
      ...
      <div className="h-full max-w-[430px] mx-auto flex flex-col" style={{ paddingTop: "env(safe-area-inset-top, 4px)", paddingBottom: "env(safe-area-inset-bottom, 4px)", paddingLeft: "16px", paddingRight: "16px" }}>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden pb-1 space-y-2 min-h-0" style={{ scrollbarWidth: "none" }}>
          ...
          {activeGuideStep === 2 && (
            <div style={{ animation: "fadeInStep 0.7s cubic-bezier(0.22, 1, 0.36, 1) both" }}>
              {/* Completed reason pill */}
              <CompletedPill text={reason} onReset={() => { setReason(""); setChiefComplaint(""); saveAnswers({ reason: "", chiefComplaint: "" }); }} />
          ...
          {activeGuideStep === 3 && (
            <div style={{ animation: "fadeInStep 0.7s cubic-bezier(0.22, 1, 0.36, 1) both" }}>
              {/* Completed pills */}
              <CompletedPill text={reason} onReset={() => { setReason(""); setChiefComplaint(""); setSymptomsDone(false); saveAnswers({ reason: "", chiefComplaint: "", symptomsDone: false }); }} />
              <div className="mt-1" />
              <CompletedPill text={`Symptoms: ${chiefComplaint}`} onReset={() => { setChiefComplaint(""); setSymptomsDone(false); saveAnswers({ chiefComplaint: "", symptomsDone: false }); }} />
              <div className="flex items-center justify-center gap-2 mt-3 mb-2">
