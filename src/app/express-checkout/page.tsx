  ...
  return (
    <div className="ec-root text-white font-sans overflow-hidden" style={{ background: "radial-gradient(900px 420px at 18% 12%, rgba(255,179,71,0.18), transparent 55%), radial-gradient(800px 380px at 76% 22%, rgba(110,231,183,0.16), transparent 55%), linear-gradient(180deg, #0b0f0c 0%, #070a08 100%)", height: "100dvh", minHeight: "100vh" }}>
      ...
      <div className="h-full max-w-[430px] mx-auto flex flex-col" style={{ paddingTop: "env(safe-area-inset-top, 4px)", paddingBottom: "env(safe-area-inset-bottom, 4px)", paddingLeft: "16px", paddingRight: "16px" }}>
        ...
        {/* ═══ CENTERED HEADER ═══ */}
        <div className={`text-center flex-shrink-0 ${headerUltraCompact ? "pt-1 pb-1" : headerIsStep5 ? "pt-2 pb-1" : "pt-3 pb-2"}`}>
          {/* Logo + Brand */}
          <div className={`flex items-center justify-center gap-2 ${headerUltraCompact ? "mb-0.5" : "mb-1"}`}>
          ...
          {headerIsStep5 && headerUltraCompact && <p className="text-gray-500 text-[9px] font-semibold mt-0.5">Review and pay to reserve your provider.</p>}

          {/* Main heading — dynamic pill text */}
          <h1 key={pillText} className={`text-white font-black leading-tight ${headerIsStep5 ? "mb-1" : "mb-2"}`} style={{ fontSize: headerIsStep5 ? "clamp(22px, 6.2vw, 28px)" : "clamp(24px, 7vw, 30px)" }}>
            {pillText}
          </h1>        </div>

        
        {/* ═══ WELCOME + PROGRESS ═══ */}
        <div className={`flex-shrink-0 ${headerIsStep5 ? "pb-1" : "pb-1.5"}`}>
          ...
          {/* Back (shows previous question only when user requests it) */}
          {uiStep > 1 && (
            <div className="mt-2 flex justify-start">
              <button
                onClick={goBack}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-white/10 bg-white/[0.03] hover:bg-white/[0.06] transition-all active:scale-95"
                style={{ fontSize: "10px", color: "rgba(233,238,247,0.85)" }}
              >
                <span style={{ fontSize: "12px", lineHeight: 1 }}>←</span> Back
              </button>
            </div>
          )}
        </div>
          ...
          {uiStep === 4.5 && (
            <div style={{ animation: "fadeInStep 0.7s cubic-bezier(0.22, 1, 0.36, 1) both" }}>
              ...
              <div className={`rounded-xl bg-transparent p-4 space-y-3 transition-all ${activeOrangeBorder}`}>
                ...
                {/* Acknowledgment — inline checkbox (same as current step 5) */}
                {visitType !== "refill" && (
                ...
                {/* Confirm & Pay button — THIS is when payment intent fires */}
                <button
                  onClick={() => {
                    handleVisitTypeChange(visitType);
                  ...
                  className="w-full py-3.5 rounded-xl text-white font-bold text-[14px] transition-all active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 border border-[#2dd4a0]"
                  style={{ background: "rgba(110,231,183,0.08)" }}
                >
                  <Lock size={14} /> Confirm &amp; Pay {currentPrice.display}
                </button>
                <p className="text-center text-gray-700 text-[8px]"><Lock size={8} className="inline mr-0.5" />No payment charged until you confirm above</p>
              </div>
            </div>
          )}
              ...
              <div className={`rounded-xl bg-transparent p-4 space-y-3 transition-all ${activeOrangeBorder}`}>
                ...
                {clientSecret && stripeOptions ? (
                  ...
                  <div className="space-y-2 py-2">
                    ...
                    <button onClick={retryPaymentIntent} className="w-full py-2.5 rounded-xl text-white font-bold text-[12px] border border-[#2dd4a0] hover:bg-[#2dd4a0]/10 transition-all" style={{ background: "rgba(110,231,183,0.05)" }}>
                      Retry Payment Setup
                    </button>
                  </div>
                ) : (
