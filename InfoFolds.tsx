  ...
  useEffect(() => {
    ...
    const handleCanPlay = () => {
      ...
      video.play().catch(err => {
        console.log('Video autoplay failed (will need user interaction):', err);
      });
    };
          ...
          <div className="relative max-w-2xl mx-auto mb-12">
            ...
            <div className="relative rounded-3xl p-6 overflow-hidden" style={{ background: '#0a1210', border: '1px solid rgba(20,184,166,0.3)' }}>
              ...
              >
                ...
                {/* PROVIDER SECTION - VIDEO BACKGROUND */}
                <div style={{ 
                  position: 'relative',
                  overflow: 'hidden',
                  ...
                  <div style={{ 
                    ...
                    <video
                      ...
                      preload="auto"
                      onError={() => {
                        // set a simple visible flag by adding an attribute to the body (debug only)
                        document.body.setAttribute('data-video-error', 'true');
                      ...
                      className="fade-in"
                    >
                      {/* Use the correct path - video is inside public/assets */}
                  ...
                  <div style={{ 
                    ...
                    width: '100%',
                    height: '100%',
                    background: 'radial-gradient(ellipse at center, rgba(5, 11, 20, 0.3) 0%, rgba(5, 11, 20, 0.8) 100%)',
                    zIndex: 1,
                    pointerEvents: 'none'
                  }}></div>
                  ...
                  <div style={{ position: 'relative', zIndex: 5, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    {/* DEBUG: show video state (visible on page for troubleshooting) */}
                    <div style={{ position: 'absolute', top: 8, right: 8, zIndex: 20 }}>
                      {videoLoaded ? (
                        <div style={{ background: 'rgba(34,197,94,0.9)', color: '#00110a', padding: '6px 8px', borderRadius: 8, fontSize: 12, fontWeight: 700 }}>Video playing</div>
                      ) : (
                        <div style={{ background: 'rgba(239,68,68,0.9)', color: '#fff', padding: '6px 8px', borderRadius: 8, fontSize: 12, fontWeight: 700 }}>Video not ready</div>
                      )}
                    </div>
                    {/* MUTED Badge */}
                ...
                <div className="px-3 pb-3 space-y-2">
                  ...
                  {/* CTA - Orange */}
                  <button 
                    className="w-full text-white px-4 py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-2 pulse-orange" 
                    style={{ background: 'linear-gradient(135deg, #f97316 0%, #ea580c 50%, #c2410c 100%)' }}
                    onClick={() => {
                      // Play video on user interaction if it's not playing
                      if (videoRef.current) {
